...
public abstract class Mapper {
    ...
    public void saveState(State s) {
        s.prg = prg.clone();
        s.chr = chr.clone();
        s.chr_map = chr_map.clone();
        s.prg_map = prg_map.clone();
        s.prgram = prgram.clone();
        s.mappertype = mappertype; s.submapper = submapper;
        s.prgsize = prgsize; s.prgoff = prgoff; s.chroff = chroff; s.chrsize = chrsize;
        s.scrolltype = scrolltype;
        s.haschrram = haschrram; s.hasprgram = hasprgram; s.savesram = savesram;
        s.pput0 = pput0.clone(); s.pput1 = pput1.clone(); s.pput2 = pput2.clone(); s.pput3 = pput3.clone();
        s.nt0 = nt0.clone(); s.nt1 = nt1.clone(); s.nt2 = nt2.clone(); s.nt3 = nt3.clone();
        s.crc = crc;
        s.region = region;
    }
    ...
    public void loadState(State s) {
        prg = s.prg.clone();
        chr = s.chr.clone();
        chr_map = s.chr_map.clone();
        prg_map = s.prg_map.clone();
        prgram = s.prgram.clone();
        mappertype = s.mappertype; submapper = s.submapper;
        prgsize = s.prgsize; prgoff = s.prgoff; chroff = s.chroff; chrsize = s.chrsize;
        scrolltype = s.scrolltype;
        haschrram = s.haschrram; hasprgram = s.hasprgram; savesram = s.savesram;
        pput0 = s.pput0.clone(); pput1 = s.pput1.clone(); pput2 = s.pput2.clone(); pput3 = s.pput3.clone();
        nt0 = s.nt0.clone(); nt1 = s.nt1.clone(); nt2 = s.nt2.clone(); nt3 = s.nt3.clone();
        crc = s.crc;
        region = s.region;
    }
    ...
    // PPU nametables
    protected int[] pput0 = new int[0x400], pput1 = new int[0x400],
    ...
    public static enum MirrorType implements Serializable {

        H_MIRROR, V_MIRROR, SS_MIRROR0, SS_MIRROR1, FOUR_SCREEN_MIRROR
    };

    public static enum TVType implements Serializable {
