...
import pl.edu.agh.ai.learning.LearningSupervisor;
...
public class SwingUI extends JFrame implements GUIInterface {
    ...
    private final PuppetController puppetController;

    private LearningSupervisor learningSupervisor;
    ...
    public SwingUI() {
        ...
        nes.setControllers(puppetController, padController2);
    ...
    public void buildMenus() {
        ...
        nesmenu.add(item = new JMenuItem("Save"));
        item.addActionListener(listener);
        item.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_F5, 0));

        nesmenu.add(item = new JMenuItem("Load"));
        item.addActionListener(listener);
        ...
        JMenu learning = new JMenu("Learning");
        learning.add(item = new JMenuItem("Init"));
        item.addActionListener(listener);

        learning.add(item = new JMenuItem("Start learning"));
        item.addActionListener(listener);

        learning.add(item = new JMenuItem("Load learning state"));
        item.addActionListener(listener);

        learning.addSeparator();

        learning.add(item = new JMenuItem("Enable keyboard control"));
        item.addActionListener(listener);

        learning.add(item = new JMenuItem("Random run"));
        item.addActionListener(listener);
    ...
    public class AL implements ActionListener, WindowListener {
        ...
        public void actionPerformed(final ActionEvent arg0) {
            ...
            if (arg0.getActionCommand().equals("Quit")) {
                ...
                nes.loadState();
            } else if (arg0.getActionCommand().equals("Init")) {
                createLearningSupervisor();
            } else if (arg0.getActionCommand().equals("Enable keyboard control")) {
                enableKeyboardControl();
            } else if (arg0.getActionCommand().equals("Random run")) {
                learningSupervisor.getLearning().enableRandomRun();
            } else if (arg0.getActionCommand().equals("Start learning")) {
                learningSupervisor.startLearning();
            } else if (arg0.getActionCommand().equals("Load learning state")) {
                createLearningSupervisor();
                try {
                    learningSupervisor.loadState();
                } catch (IOException e) {
                    e.printStackTrace();
                }
            }
    ...
    private void createLearningSupervisor() {
        learningSupervisor = new LearningSupervisor(nes);
        learningSupervisor.init();
    }
}
