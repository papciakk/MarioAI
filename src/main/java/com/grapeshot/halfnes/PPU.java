...
public class PPU {
    ...
    public void saveState(State s) {
        s.oamaddr = oamaddr; s.oamstart = oamstart; s.readbuffer = readbuffer;
        s.loopyV = loopyV; s.loopyT = loopyT; s.loopyX = loopyX;
        s.scanline = scanline;
        s.cyclesPPU = cycles; s.framecount = framecount;
        s.div = div;
        s.OAM = OAM.clone(); s.secOAM = secOAM.clone();
        s.spriteshiftregH = spriteshiftregH.clone(); s.spriteshiftregL = spriteshiftregL.clone();
        s.spriteXlatch = spriteXlatch.clone(); s.spritepals = spritepals.clone();
        s.found = found;
        s.bgShiftRegH = bgShiftRegH; s.bgShiftRegL = bgShiftRegL;
        s.bgAttrShiftRegH = bgAttrShiftRegH; s.bgAttrShiftRegL = bgAttrShiftRegL;
        s.bgShiftRegH_ai = bgShiftRegH_ai; s.bgShiftRegL_ai = bgShiftRegL_ai;
        s.spritebgflags = spritebgflags.clone();
        s.even = even;
        s.bgpattern = bgpattern; s.sprpattern = sprpattern;
        s.spritesize = spritesize; s.nmicontrol = nmicontrol;
        s.grayscale = grayscale;
        s.bgClip = bgClip; s.spriteClip = spriteClip;
        s.bgOn = bgOn; s.spritesOn = spritesOn;
        s.vblankflag = vblankflag; s.sprite0hit = sprite0hit; s.spriteoverflow = spriteoverflow;
        s.emph = emph;
        s.pal = pal.clone();
        s.vraminc = vraminc;
        s.bgcolors = bgcolors.clone();
        s.openbus = openbus;
        s.nextattr = nextattr;
        s.linelowbits = linelowbits; s.linehighbits = linehighbits;
        s.linelowbits_ai = linelowbits_ai; s.linehighbits_ai = linehighbits_ai;
        s.penultimateattr = penultimateattr;
        s.numscanlines = numscanlines;
        s.vblankline = vblankline;
        s.cpudivider = cpudivider.clone();
    }

    public void loadState(State s) {
        oamaddr = s.oamaddr; oamstart = s.oamstart; readbuffer = s.readbuffer;
        loopyV = s.loopyV;
        loopyT = s.loopyT;
        loopyX = s.loopyX;
        scanline = s.scanline;
        ...
        div = s.div;
        OAM = s.OAM.clone(); secOAM = s.secOAM.clone();
        spriteshiftregH = s.spriteshiftregH.clone(); spriteshiftregL = s.spriteshiftregL.clone();
        spriteXlatch = s.spriteXlatch.clone(); spritepals = s.spritepals.clone();
        found = s.found;
        bgShiftRegH = s.bgShiftRegH; bgShiftRegL = s.bgShiftRegL;
        bgAttrShiftRegH = s.bgAttrShiftRegH; bgAttrShiftRegL = s.bgAttrShiftRegL;
        bgShiftRegH_ai = s.bgShiftRegH_ai; bgShiftRegL_ai = s.bgShiftRegL_ai;
        spritebgflags = s.spritebgflags.clone();
        even = s.even;
        bgpattern = s.bgpattern; sprpattern = s.sprpattern;
        spritesize = s.spritesize; nmicontrol = s.nmicontrol;
        grayscale = s.grayscale;
        bgClip = s.bgClip; spriteClip = s.spriteClip;
        bgOn = s.bgOn; spritesOn = s.spritesOn;
        vblankflag = s.vblankflag; sprite0hit = s.sprite0hit; spriteoverflow = s.spriteoverflow;
        emph = s.emph;
        pal = s.pal.clone();
        vraminc = s.vraminc;
        bgcolors = s.bgcolors.clone();
        openbus = s.openbus;
        nextattr = s.nextattr;
        linelowbits = s.linelowbits; linehighbits = s.linehighbits;
        linelowbits_ai = s.linelowbits_ai; linehighbits_ai = s.linehighbits_ai;
        penultimateattr = s.penultimateattr;
        numscanlines = s.numscanlines;
        vblankline = s.vblankline;
        cpudivider = s.cpudivider.clone();
    }
    ...
    private final int[] bitmap_ai = new int[240 * 256];
    private int found, bgShiftRegH, bgShiftRegL, bgAttrShiftRegH, bgAttrShiftRegL;
    private int bgShiftRegH_ai, bgShiftRegL_ai;
    ...
    private final static boolean PPUDEBUG = true;
    ...
    private int linelowbits, linehighbits;
    private int linelowbits_ai, linehighbits_ai;
    ...
    public BufferedImage getNametableView() {
        return nametableView;
    }

    public int[] getBitmap_ai() {
        return bitmap_ai;
    }
    ...
    public PPU(final Mapper mapper) {
     ...
     expecting this set of values and nesemu1, BizHawk, RockNES, MyNes use it
        ...
        fill(OAM, 0xff);
        if (PPUDEBUG) {
            nametableView = new BufferedImage(256, 240, TYPE_INT_BGR);
            debuggui = new DebugUI(256, 240);
            debuggui.run();
        }
        setParameters();
    }
...
//        }
    ...
    private void bgFetch() {
        ...
        switch ((cycles - 1) & 7) {
            ...
            case 5:
                ...
                TileType tile = t.getBgTileById(tileId);
                int val = tile.getVal() & 1;

                linelowbits_ai = val == 1 ? 255 : 0;
                break;
            case 7:
                //fetch high bg byte
                ...
                t = new SuperMarioBrosTiles();
                tile = t.getBgTileById(tileId);
                val = tile.getVal() & 2;

                linehighbits_ai = val > 0 ? 255 : 0;

                bgShiftRegL_ai |= linelowbits_ai;
                bgShiftRegH_ai |= linehighbits_ai;
    ...
    private void fetchNTByte() {
        //fetch nt byte
        int addr = ((loopyV & 0xc00) | 0x2000) + (loopyV & 0x3ff);
        tileId = mapper.ppuRead(addr);
        tileAddr = tileId * 16 + (bgpattern ? 0x1000 : 0);
    }
    ...
    private boolean drawBGPixel(int bufferoffset) {
        ...
        if (bgClip && (bufferoffset & 0xff) < 8) {
            ...
            final int bgPix_ai = (((bgShiftRegH_ai >> -loopyX + 16) & 1) << 1)
                    + ((bgShiftRegL_ai >> -loopyX + 16) & 1);
            ...
            bitmap_ai[bufferoffset] = bgPix_ai;
...
//        SuperMarioBrosTiles tiles = new SuperMarioBrosTiles();
//
//        for (int i = 0; i < 32; ++i) {
//            for (int j = 0; j < 30; ++j) {
//                int tileId = mapper.ppuRead(0x2000 + i + 32 * j);
//                TileType tile = tiles.getBgTileById(tileId);
//
//                for (int ii = 0; ii < 8; ii++) {
//                    for (int jj = 0; jj < 8; jj++) {
//                        nametableView.setRGB(i * 8 + ii + 255, j * 8 + jj + 240, tile.getColor().getRGB());
//                    }
//                }
//            }
//        }
